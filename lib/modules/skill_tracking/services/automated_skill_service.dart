...
class AutomatedSkillService {
  ...
  final SkillTrackingRepository _skillRepository = SkillTrackingRepository();
  ...
  Future<void> onBookingCompleted({
    ...
    try {
      if (kDebugMode) {
        debugPrint('🎯 AutomatedSkillService: Processing booking completion for user $userId');
      }
      ...
      final updateResult = SkillUpdateRules.calculateBookingUpdate(
        ...
        context: 'Completed session: $sessionTitle (${sessionDurationHours.toStringAsFixed(1)}h)',
        metadata: {
          'bookingId': bookingId,
      ...
      if (updateResult.hasChanges) {
        ...
        if (kDebugMode) {
          debugPrint('✅ AutomatedSkillService: Applied booking skill updates for user $userId');
        }
      }
    } catch (e) {
      if (kDebugMode) {
        debugPrint('❌ AutomatedSkillService: Error processing booking completion: $e');
      }
  ...
  Future<void> onVenueBookingCompleted({
    ...
    try {
      if (kDebugMode) {
        debugPrint('🎯 AutomatedSkillService: Processing venue booking completion for user $userId');
      }
      ...
      final updateResult = SkillUpdateRules.calculateBookingUpdate(
        ...
        sessionDurationHours: sessionDurationHours,
        recentSessionsCount: recentSessionsCount,
        context: 'Completed venue session: $venueTitle (${sessionDurationHours.toStringAsFixed(1)}h)',
        metadata: {
      ...
      if (updateResult.hasChanges) {
        ...
        if (kDebugMode) {
          debugPrint('✅ AutomatedSkillService: Applied venue booking skill updates for user $userId');
        }
      }
    } catch (e) {
      if (kDebugMode) {
        debugPrint('❌ AutomatedSkillService: Error processing venue booking completion: $e');
      }
      // Don't throw - skill updates shouldn't break the booking flow
    }
  }
  ...
  Future<void> onTournamentCompleted({
    ...
    try {
      if (kDebugMode) {
        debugPrint('🎯 AutomatedSkillService: Processing tournament completion for user $userId');
      }
      ...
      final updateResult = SkillUpdateRules.calculateTournamentUpdate(
        ...
        didWin: didWin,
        context: 'Tournament ${didWin ? 'victory' : 'participation'}: $tournamentName',
        metadata: {
      ...
      if (updateResult.hasChanges) {
        ...
        if (kDebugMode) {
          debugPrint('✅ AutomatedSkillService: Applied tournament skill updates for user $userId');
        }
      }
    } catch (e) {
      if (kDebugMode) {
        debugPrint('❌ AutomatedSkillService: Error processing tournament completion: $e');
      }
  ...
  Future<void> onFeedbackReceived({
    ...
    try {
      if (kDebugMode) {
        debugPrint('🎯 AutomatedSkillService: Processing feedback for user $userId');
      }
      ...
      if (updateResult.hasChanges) {
        ...
        if (kDebugMode) {
          debugPrint('✅ AutomatedSkillService: Applied feedback skill updates for user $userId');
        }
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> applyInactivityDecay(String userId) async {
    try {
      if (kDebugMode) {
        debugPrint('🎯 AutomatedSkillService: Checking inactivity decay for user $userId');
      }
      ...
      if (updateResult.hasChanges) {
        ...
        if (kDebugMode) {
          debugPrint('✅ AutomatedSkillService: Applied inactivity decay for user $userId');
        }
      }
    } catch (e) {
      if (kDebugMode) {
        debugPrint('❌ AutomatedSkillService: Error applying inactivity decay: $e');
      }
    }
  }
  ...
  Future<int> _getDaysSinceLastActivity(String userId) async {
    try {
      ...
      if (recentBooking.docs.isNotEmpty) {
        ...
        final completedAt = (lastBooking['completedAt'] as Timestamp?)?.toDate();
        if (completedAt != null) {
  ...
  Future<Map<SkillType, int>> _getCurrentSkillScores(String userId) async {
    try {
      ...
      return {
        for (final skillType in SkillType.values) skillType: 50
      };
    }
  }

  /// Apply skill update by creating a new skill log
  Future<void> _applySkillUpdate(String userId, SkillUpdateResult updateResult) async {
    // Get current skill scores
    final currentSkills = await _getCurrentSkillScores(userId);
    ...
    await _skillRepository.addSkillLog(skillLog);
  }
