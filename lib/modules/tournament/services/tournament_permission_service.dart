  ...
  Future<TournamentCreationPermission> checkTournamentCreationPermission(
    ...
    try {
      ...
      return const TournamentCreationPermission(
        canCreate: false,
        reason:
            'Only Team Captains, Coaches, or Venue Owners can create tournaments',
      );
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<bool> _checkIfUserIsTeamCaptain() async {
    try {
      ...
      for (final team in teams) {
        ...
        if (userMember != null &&
            (userMember.role.name == 'captain' ||
                userMember.role.name == 'owner')) {
          return true;
        }
      }
...
extension TournamentCreationPermissionTypeExtension
    on TournamentCreationPermissionType {
